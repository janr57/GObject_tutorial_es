...
En sentido amplio, \textsf{GObject} es un objeto, que proporciona dos entidades relacionadas:
instancias y clases. La función \texttt{g\_object\_new} se encarga de crearlas:
\begin{itemize}
  \tightlist
\item Una clase \textsf{GObject} ---\textsf{GObjectClass}--- se crea la primera vez que se ejecuta la función
  ...
  Por ello se crea la primera vez que se ejecuta la función.
  La clase se materializa en una estructura en lenguaje C, que la representa, pero es compleja y no es
  conveniente mostrarla aquí. Solo decir que la primera vez que se llama a \texttt{g\_object\_new} se reserva
  espacio para esa estructura \textsf{GObjectClass} y se inicializa.
...
\begin{lstlisting}[language=bash]
  ...
  $ meson setup build [--buildtype=release|debug|debugoptimized]
  $ ninja -C build
\end{lstlisting}
