...
\item Declara una función de inicialización de la clase. Su nombre es \textsf{<espacio-de-nombres>\_<nombre>\_class\_init}. Esto es una declaración, no una definición,
  ...
  Por ejemplo, para el objeto \textsf{TDouble}, la función sería \texttt{t\_doble\_class\_init}.
...
\item Define una variable estática que apunta a la clase de la que deriva. Su nombre es
  ...
  Por ejemplo, para el objeto \textsf{TDouble}, la variable es \texttt{t\_doble\_parent\_class}.
\item Define una función \textsf{<espacio-de-nombres>\_<nombre>\_get\_type ()}.
  
  Por ejemplo para el objeto \textsf{TDouble}, la función es \texttt{t\_doble\_get\_type}.
...
\texttt{G\_DEFINE\_TYPE (TDouble, t\_doble, G\_TYPE\_OBJECT)}.
...
\item Línea 22:\par
  ...
  En este ejemplo, los argumentos son: \texttt{TDouble}, \texttt{t\_doble} y \texttt{G\_TYPE\_OBJECT}.
...
\texttt{\#define T\_TYPE\_DOUBLE (t\_doble\_get\_type ())} antes que \texttt{G\_DECLARE\_FINAL\_TYPE}.
...
\item Líneas 9--10:\par
  ...
  \passthrough{\lstinline!t\_doble\_get\_type!} y, después se declara el tipo \textsf{TDouble} por medio
...
Se estructura un programa que usa objetos \textsf{TDouble}, en ficheros individuales \textsf{tdoble.h},
\textsf{tdoble.c} y \textsf{main.c}.
El objeto \textsf{TDouble} se declara y define en los dos primeros ficheros, respectivamente.
Ver instrucciones para compilar los programas en la sección \ref{sec:compilacion}.

\subsubsection{Declaración del objeto \textsf{TDouble}: fichero de cabecera \textsf{tdoble.h}}
...
\item Líneas 11--12:\par
  Se declara la macro \texttt{T\_TYPE\_DOUBLE} que se expandirá a \texttt{t\_doble\_get\_type ()}.
  Se declara el tipo de objeto \textsf{TObject}, mediante la macro\texttt{G\_DECLARE\_FINAL\_TYPE}
  a la que se añaden los argumentos: \texttt{TDouble}, \texttt{t\_doble} y \texttt{GObject}.
...
\subsubsection{Definición del objeto \textsf{TDouble}: fichero \textsf{tdoble.c}}
...
\item Líneas 8--11:\par
  Estructura de los objetos \textsf{TDouble}. Su nombre debe ser \texttt{\_TDouble} porque la macro
  ...
  \par
  Al declarar la estructura en este fichero, se impide asignar el valor del parámetro \texttt{value}
  de la misma mediante el código \texttt{obj->value = 10.0}, por ejemplo, o leer su valor mediante un
  código del estilo de \texttt{double var = obj->value}. En su lugar, para asignar valores, se hará mediante
  el constructor \texttt{t\_double\_new (10.0)}, por ejemplo, o mediante la función \emph{setter},
  \texttt{t\_doble\_set\_value (obj, 10.0)}, y para leer valores, se utilizará la función \emph{getter}
  \texttt{t\_doble\_get\_value (obj, \&var)}, donde \texttt{var} es una variable de tipo \texttt{double}.
  Estas funciones comprueban que \texttt{obj} sea una instancia de \textsf{TDoble}.
  \par
...
\item Línea 48:\par
  ...
  \texttt{T\_TYPE\_DOUBLE} se expande a la función \passthrough{\lstinline!t\_doble\_get\_type ()!}.
...
Es natural añadir operadores de cálculo a los objetos \textsf{TDouble} porque representan números reales.
