...
Este ejemplo se puede compilar de dos formas diferentes:
\begin{itemize}
  \item Utilizando \textsf{make}:
\begin{lstlisting}[language=bash]
  $ cd prog/ej06
  $ make
\end{lstlisting}
\item Utilizando \textsf{ninja}:
\begin{lstlisting}[language=bash]
  $ cd prog/ej06
  $ meson setup _build
  $ ninja -C _build
\end{lstlisting}
\end{itemize}
Para borrar el ejecutable y ficheros auxiliares:
\begin{lstlisting}[language=bash]
  $ cd prog/ej06
  $ make clean
\end{lstlisting}
...
\item Líneas 23--29:\par
  ...
  Este registro le sirve al programador para informar del error, pero no se debe utilizar para informar del
  error al usuario. Para más información, ver \href{https://docs.gtk.org/glib/error-reporting.html}
  ...
  conocen el tipo de los parámetros.
\item Líneas 31--36:\par
